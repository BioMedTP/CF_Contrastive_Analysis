...
class ImageDatasetFlexible(Dataset):
    def __init__(self, root, transform=None):
        ...
        if root.endswith(".npy"):
            # 1. load paths
            paths = np.load(root, allow_pickle=True)

            # 2. flatten + str
            paths = np.array([str(p) for p in paths.reshape(-1)], dtype=object)

            # 3. keep only existing files (和 baseline 对齐)
...
class ImagesDataset_mednpy(Dataset):
    ...
    def __init__(self, root, transform):
        ...
        self.paths = np.load(root, allow_pickle=True).tolist()
        self.transform = transform
