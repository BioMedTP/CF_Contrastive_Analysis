...
class BaseRunner:
    ...
    def _setup_method(self):
        ...
        if method_name == "fse_full":
            self.method = methods_registry[method_name](
                checkpoint_path=self.config.model.checkpoint_path,
                w_space_encoder=self.config.model.w_space_encoder,
                stylegan_size = self.config.model.stylegan_size,
                dataset = self.config.data.dataset,
                pSp_cs_path = self.config.model.pSp_cs_path,
                pSp_path = self.config.model.pSp_path,
                inverter_pth=self.config.model.inverter_pth,
                channel_multiplier = self.config.model.channel_multiplier,
                **self.config.methods_args[method_name],
            ).to(self.device)
        elif method_name == "fse_inverter":
            self.method = methods_registry[method_name](
                checkpoint_path=self.config.model.checkpoint_path,
                dataset = self.config.data.dataset,
                stylegan_size = self.config.model.stylegan_size,
