...
class FSEInferenceRunner(BaseInferenceRunner):
    def _run_on_batch(self, inputs):
        ...
        if self.config.model.w_space_encoder == "e4e":
            # print("Using e4e encoder")
            w_e4e = self.method.e4e_encoder(x)
            w_e4e = w_e4e + self.method.latent_avg
        elif self.config.model.w_space_encoder == "pSp":
            # print("Using pSp encoder")
            w_e4e = self.method.pSp_encoder(x, return_latents=True)[1]
    ...
    def _run_editing_on_batch(self, method_res_batch, editing_name, editing_degrees, mask=None, return_e4e=False):
        ...
        for i, latent in enumerate(orig_latents):
            ...
            e4e_edited_images.append(e4e_edit)
            
        edited_images = torch.stack(edited_images)
        e4e_inv_images = torch.stack(e4e_inv_images)
        e4e_edited_images = torch.stack(e4e_edited_images)        

        if return_e4e:
            return edited_images, e4e_inv_images, e4e_edited_images
