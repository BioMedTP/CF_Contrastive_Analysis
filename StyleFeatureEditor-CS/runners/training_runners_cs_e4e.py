...
class BaseTrainingRunner(BaseRunner):
    ...
    def calculate_delta(self, x_resh, x_cond_resh):
        ...
        e4e_c_x, e4e_s_x = self.method.e4e_cs_model(w_e4e_x)
        e4e_c_y, e4e_s_y = self.method.e4e_cs_model(w_e4e_x_cond)
    ...
    def swap_by_delta(self, x, x_cond, n_iter=1e5):
        ...
        delta, x_E, y_E = self.calculate_delta(x_resh, x_cond_resh)
    ...
    def inference_special(self):
        ...
        rec_X = self._run_on_batch(X)
        rec_Y = self._run_on_batch(Y)

        swap_X2Y, e4e_rec_X, e4e_swap_X2Y = self.swap_by_delta(X, Y)
        swap_Y2X, e4e_rec_Y, e4e_swap_Y2X = self.swap_by_delta(Y, X)

        idx = self.config.data.special_idx

        row1 = torch.stack([X[idx], e4e_rec_X[idx], e4e_swap_X2Y[idx], rec_X[idx], swap_X2Y[idx]], dim=0)
        row2 = torch.stack([Y[idx], e4e_rec_Y[idx], e4e_swap_Y2X[idx], rec_Y[idx], swap_Y2X[idx]], dim=0)
        ...
        visualize_batch_grid(
            ...
            titles=["Input", "e4e-cs Recon", "e4e-cs Swap", "Refined Recon", "Refined Swap"],
...
class FSEEditorCSTrainingRunner(BaseTrainingRunner):
    def forward(self, x, x_cond):
        ...
        with torch.no_grad():
            ...
            e4e_c_x, e4e_s_x = self.method.e4e_cs_model(w_e4e_x)
            _, e4e_s_y = self.method.e4e_cs_model(w_e4e_x_cond)
