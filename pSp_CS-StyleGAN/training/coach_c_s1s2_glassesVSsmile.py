...
from datasets.images_dataset import ImagesDataset_npy
...
class Coach_csmlp:
	...
	def inference_special(self, x_bg, x_t, name):
		...
		with torch.no_grad():
			...
			swap_cs_X = self.pSp_net.forward(latent_bg_c + latent_t_s2, input_code=True, randomize_noise=True, recon_modle=True)
			swap_cs_Y = self.pSp_net.forward(latent_t_c + latent_bg_s1, input_code=True, randomize_noise=True, recon_modle=True)	

		idx = self.opts.special_idx
			
		row1 = torch.stack([x_bg[idx], rec_x_pSp_bg[idx], rec_x_bg[idx], swap_cs_X[idx]], dim=0)
		row2 = torch.stack([x_t[idx], rec_x_pSp_t[idx], rec_x_t[idx], swap_cs_Y[idx]], dim=0)
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				w_bg_pSp.requires_grad_(True)
				w_t_pSp.requires_grad_(True)
	...
	def configure_datasets(self):
		...
		train_bg_dataset = ImagesDataset_npy(source_root=dataset_args['train_bg_source_root'],
									  target_root=dataset_args['train_bg_target_root'],
									  source_transform=transforms_dict['transform_source'],
									  target_transform=transforms_dict['transform_train'],
									  opts=self.opts)
		
		train_t_dataset = ImagesDataset_npy(source_root=dataset_args['train_t_source_root'],
									  target_root=dataset_args['train_t_target_root'],
									  source_transform=transforms_dict['transform_source'],
									  target_transform=transforms_dict['transform_train'],
									  opts=self.opts)
				
		test_bg_dataset = ImagesDataset_npy(source_root=dataset_args['test_bg_source_root'],
									 target_root=dataset_args['test_bg_target_root'],
									 source_transform=transforms_dict['transform_source'],
									 target_transform=transforms_dict['transform_test'],
									 opts=self.opts)
		
		test_t_dataset = ImagesDataset_npy(source_root=dataset_args['test_t_source_root'],
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s1, latent_bg_s2, latent_t_c, latent_t_s1, latent_t_s2, w_bg_pSp, w_t_pSp):
		...
		if self.opts.sbg_lambda > 0:
			...
			loss_silent_1 = F.mse_loss(latent_bg_s1, torch.zeros_like(latent_bg_s1).to(self.device))
			loss_silent_2 = F.mse_loss(latent_bg_s2, torch.zeros_like(latent_bg_s2).to(self.device))
			loss_dict['loss_silent_bg'] = float(loss_silent_1 + loss_silent_2)
			loss += (loss_silent_1 + loss_silent_2) * self.opts.sbg_lambda
