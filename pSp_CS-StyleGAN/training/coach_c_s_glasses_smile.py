...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.special_batch_x, self.special_batch_y1, self.special_batch_y2, self.special_batch_y3 = get_special_glasses_smile_images(self.device, size=self.opts.transform_size)
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				if self.global_step % self.opts.image_interval == 0 :
					...
					self.inference_special(self.special_batch_x, self.special_batch_y1, name="val_glasses")
					self.inference_special(self.special_batch_x, self.special_batch_y2, name="val_smile")
					self.inference_special(self.special_batch_x, self.special_batch_y3, name="val_both")
