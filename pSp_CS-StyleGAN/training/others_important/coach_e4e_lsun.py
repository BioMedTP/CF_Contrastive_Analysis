...
from criteria_e4e import id_loss
from configs import data_configs
from datasets.images_dataset import ImagesDataset
from criteria_e4e.lpips.lpips import LPIPS
from models.e4e_models.psp import pSp
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				rec_x_e4e_bg, w_bg_pSp = self.pSp_net.forward(x_bg, randomize_noise=False, return_latents=True)
				rec_x_e4e_t, w_t_pSp = self.pSp_net.forward(x_t, randomize_noise=False, return_latents=True) 
				...
				rec_x_bg, _ = self.pSp_net.decoder([latent_bg_c], randomize_noise=False, input_is_latent=True)
				rec_x_t, _ = self.pSp_net.decoder([latent_t_c + latent_t_s], randomize_noise=False, input_is_latent=True)	
				...
				if self.global_step % self.opts.image_interval == 0 :
					self.parse_and_log_images(id_logs_bg, x_bg, rec_x_e4e_bg, rec_x_bg, title='images/train/faces_bg')
					self.parse_and_log_images(id_logs_t, x_t, rec_x_e4e_t, rec_x_t, title='images/train/faces_t')
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_t_dataloader)):
			...
			with torch.no_grad():
				...
				_, w_bg_pSp = self.pSp_net.forward(x_bg, randomize_noise=False, return_latents=True)
				_, w_t_pSp = self.pSp_net.forward(x_t, randomize_noise=False, return_latents=True) 
