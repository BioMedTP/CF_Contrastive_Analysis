...
class Coach_csmlp:
	...
	def	init_cls_network(self, prev_train_path):
		...
		if self.opts.disc_type == 'global':
			self.mi_model = DiscriminatorGlobal(self.opts).to(self.device)
		elif self.opts.disc_type == 'indept':
			self.mi_model = Discriminator(self.opts).to(self.device)

		# Load weights
		print(f"Loaded mi model from {prev_train_path}")
		self.mi_model.load_state_dict(torch.load(prev_train_path, map_location=self.device))
		self.mi_model.eval()  # Set to evaluation mode
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				log_msg = (f"Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | " f"g_mi_loss: {g_mi_loss:.4f} | "\
				...
				loss = g_mi_loss * self.opts.w_mi_lambda + loss_lat + loss_img_bg + loss_img_t
	...
	def validate(self):
		...
		log_msg = (f"Val Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | " f"g_mi_loss: {avg_g_loss_mi:.4f} | "\
