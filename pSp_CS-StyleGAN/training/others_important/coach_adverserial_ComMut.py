...
from models.MI_discriminator import DiscriminatorGlobal
...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.init_mut_network(prev_train_path = self.opts.mut_checkpoint_path)
	...
	def	init_cls_network(self, prev_train_path):
		...
		self.mut_model = DiscriminatorGlobal(self.opts).to(self.device)
		# Load weights
		print(f"Loaded mi model from {prev_train_path}")
		self.mut_model.load_state_dict(torch.load(prev_train_path, map_location=self.device))
		self.mut_model.eval()  # Set to evaluation mode
		# test_model_on_latents(output_latents_1, model, device)
	...
	def calc_mi_loss(self, latent_t_c, latent_t_s):

		# **Directly pass inputs to the discriminator**
		D_real = self.mi_model(latent_t_c, latent_t_s)  # Discriminator processes (batch, 18, 512)

		if self.opts.mi_loss_method == "bce":
			# **Force D_real to 0 (make D fail)**
			g_loss = self.criterion(D_real, torch.zeros_like(D_real))  # Adversarial loss
		elif self.opts.mi_loss_method == "relu":
			# **Direct MI minimization (density-ratio trick)**
			D_real = torch.clamp(D_real, 1e-4, 1 - 1e-4)  # Prevents log(0) issues
			g_loss = torch.relu(torch.log(D_real) - torch.log(1 - D_real)).mean()
		else:
			raise ValueError(f"Invalid method: {self.opts.mi_loss_method}. Choose 'bce' or 'relu'.")
		
		with torch.no_grad():
			# **Generate fake pairs by shuffling `latent_s_t`**
			perm = torch.randperm(latent_t_c.shape[0])
			shuffled_t_s = latent_t_s[perm]
			D_fake = self.mi_model(latent_t_c, shuffled_t_s)  # Fake pairs (q(c) q(s))
			# **BCE Loss**
			d_loss_real = self.criterion(D_real, torch.ones_like(D_real))  # Train D to classify real as 1
			d_loss_fake = self.criterion(D_fake, torch.zeros_like(D_fake))  # Train D to classify fake as 0		
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				# Compute classification loss (adversarial)
				g_mi_loss, d_loss_real, d_loss_fake = self.calc_mi_loss(latent_t_c, latent_t_s)
				...
				log_msg = (f"Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | "
						...
						f"G_mi_loss: {g_mi_loss:.4f}, d_loss_real: {d_loss_real:.4f}, d_loss_fake: {d_loss_fake:.4f} \n")
				...
				loss = (loss_cls + g_mi_loss) * self.opts.w_cls_lambda + loss_lat + loss_img_bg + loss_img_t
	...
	def validate(self):
		...
		total_g_mi_loss, total_d_loss_real, total_d_loss_fake = 0, 0, 0
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_t_dataloader)):
			...
			with torch.no_grad():
				...
				total_g_mi_loss += g_mi_loss.item()
		...
		# Ensure all values are standard Python floats
		avg_g_loss_mi = float(total_g_mi_loss / num_batches)
		avg_d_loss_real = float(total_d_loss_real / num_batches)
		avg_d_loss_fake = float(total_d_loss_fake / num_batches)
		...
		log_msg = (f"Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | "
				...
				f"G_mi_loss: {avg_g_loss_mi:.4f}, d_loss_real: {avg_d_loss_real:.4f}, d_loss_fake: {avg_d_loss_fake:.4f} \n")
