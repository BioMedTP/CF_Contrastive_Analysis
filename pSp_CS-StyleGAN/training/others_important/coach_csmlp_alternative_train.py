...
from datasets.inference_dataset import LatentClassifierDataset_Buffer
...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.optimizer, self.optimizer_cls = self.configure_optimizers()
		...
		self.cls_checkpoint_dir = os.path.join(opts.exp_dir, 'cls_checkpoints')
	...
	def init_cls_network(self, prev_train_path):
		"""
		Initializes the classifier model and loads weights only if a valid checkpoint path is provided.
		"""
		self.cls_model = CustomLatentClassifier(input_dim=18 * 512, num_layers=2, hidden_dim=None).to(self.device)

		# Check if a checkpoint path is provided and is valid
		if prev_train_path and os.path.isfile(prev_train_path):
			...
			print("No cls_model checkpoint provided. Initializing classifier from scratch.")
	...
	def collect_latents_dataloader(self, bg_dataloader, t_dataloader, batch_size=4, num_workers=4, shuffle=True):

		print("Collecting latents for classifier training...")

		  # Ensure MLP is in evaluation mode
		latent_list = []
		label_list = []

		# Process background images
		with torch.no_grad():
			for batch_bg in bg_dataloader:
				x_bg, _ = batch_bg  # Extract images
				...
				latent_list.append(latent_bg_c.cpu())  # Shape: (batch_size, 18, 512)
				# Store labels (0 for background)
				label_list.append(torch.zeros(latent_bg_c.shape[0], dtype=torch.long))

		# Process target images
		with torch.no_grad():
			for batch_t in t_dataloader:
				x_t, _ = batch_t  # Extract images
				...
				latent_list.append(latent_t_c.cpu())  # Shape: (batch_size, 18, 512)
				# Store labels (1 for target)
				label_list.append(torch.ones(latent_t_c.shape[0], dtype=torch.long))

		# Convert lists to tensors
		if latent_list:
			latents = torch.cat(latent_list, dim=0)  # Shape: (N, 18, 512)
		else:
			latents = torch.empty(0, 18, 512)
		...
		print(f"Collected {latents.shape[0]} total latents for classifier.")
	...
	def train_classifier(self):
		...
		train_cls_dataloader = self.collect_latents_dataloader(
			self.train_bg_dataloader, self.train_t_dataloader, batch_size=4, shuffle=True
		)

		# Collect validation data
		val_cls_dataloader = self.collect_latents_dataloader(
			self.test_bg_dataloader, self.test_t_dataloader, batch_size=4, shuffle=False
		)

		# Train classifier for `opts.cls_num_epochs` epochs
		for epoch in range(self.opts.cls_num_epochs):
			total_loss = 0
			correct = 0
			total = 0

			for latent_c, labels in train_cls_dataloader:
				latent_c, labels = latent_c.to(self.device), labels.float().to(self.device)

				# Forward pass
				outputs = self.cls_model(latent_c).squeeze()
			...
			# Log training loss
			if (epoch + 1) % self.opts.cls_val_interval == 0 or (epoch + 1) == self.opts.cls_num_epochs:
				val_loss, val_acc = self.validate_classifier(val_cls_dataloader)
				log_msg = (f"Epoch [{epoch+1}/{self.opts.cls_num_epochs}] | "
						f"Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f} | "
						f"Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}\n")
				self.write_log_to_txt(log_msg, "classifier_train_val_loss.txt")
				print(log_msg)
			...
			if (epoch + 1) % self.opts.cls_save_interval == 0 or (epoch + 1) == self.opts.cls_num_epochs:
				checkpoint_path = f"{self.cls_checkpoint_dir}/cls_model_epoch_{epoch+1}.pth"
				torch.save(self.cls_model.state_dict(), checkpoint_path)
	...
	def validate_classifier(self, val_cls_dataloader):
		"""
		...
		with torch.no_grad():
			for latent_c, labels in val_cls_dataloader:
				latent_c, labels = latent_c.to(self.device), labels.float().to(self.device)
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			# **Only at step 1000: Store latents for classifier training**
			if self.global_step % self.opts.train_cls_interval == 0:

				print(f"Training classifier on collected latents for step {self.global_step}...")
				self.train_classifier()
				self.cls_update_count = 0
				print("Classifier training completed. Resuming main training...")
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				if self.cls_update_count < self.opts.max_cls_steps == 0:
					...
					self.cls_update_count +=1
				else:
					loss_cls, loss_d_bg, loss_d_t, probs_bg, probs_t = 0.0, 0.0, 0.0, 0.0

				train_loss_dict = self.merge_loss_dict(loss_lat_dict, loss_img_dict_bg, loss_img_dict_t)
				loss = loss_lat + loss_img_bg + loss_img_t + loss_cls * self.opts.w_cls_lambda
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_t_dataloader)):
			...
			agg_loss_cls.append(loss_img_dict_t)
	...
	def configure_optimizers(self):
		...
		optimizer_cls = torch.optim.Adam(self.cls_model.parameters(), lr=self.opts.cls_lr)

		return optimizer, optimizer_cls
