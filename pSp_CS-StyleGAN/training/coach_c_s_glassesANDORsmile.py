...
from utils.data_utils import get_special_glasses_smile_images
...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.special_batch_x, self.special_batch_y1, self.special_batch_y2, self.special_batch_y3 = get_special_glasses_smile_images(self.device, size=self.opts.transform_size, datasets=self.opts.dataset_type)
	...
	def inference_special(self, x_bg, x_t, name):
		...
		with torch.no_grad():
			...
			swap_cs_X = self.pSp_net.forward(latent_bg_c + latent_t_s, input_code=True, randomize_noise=True, recon_modle=True)
			swap_cs_Y = self.pSp_net.forward(latent_t_c, input_code=True, randomize_noise=True, recon_modle=True)	
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				if self.global_step % self.opts.image_interval == 0 :
					...
					self.inference_special(self.special_batch_x, self.special_batch_y1, name="val_xy1")
					self.inference_special(self.special_batch_x, self.special_batch_y2, name="val_xy2")
					self.inference_special(self.special_batch_x, self.special_batch_y3, name="val_xy3")
					self.cs_mlp_net.train()					
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp):
		...
		if self.opts.lat_recon_lambda > 0:
			loss_lat_recon_bg = F.mse_loss(latent_bg_c + latent_bg_s, w_bg_pSp)
			loss_lat_recon_t = F.mse_loss(latent_t_c + latent_t_s, w_t_pSp)
