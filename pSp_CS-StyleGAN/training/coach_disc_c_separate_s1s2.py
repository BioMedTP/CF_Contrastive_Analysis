...
from base_functions.train_funcs import train_common_disc, calc_adv_loss
import math
...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.disc_net = CustomLatentClassifier(input_dim=self.opts.style_dim*512, num_layers=opts.disc_n_layers).to(self.device)

		self.optimizer_disc = torch.optim.Adam(self.disc_net.parameters(), lr=opts.disc_lr)
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				rec_x_pSp_bg, w_bg_pSp = self.pSp_net.forward(x_bg, return_latents=True)
				rec_x_pSp_t, w_t_pSp = self.pSp_net.forward(x_t, return_latents=True) 

				latent_bg_c, latent_bg_s1, latent_bg_s2 = self.cs_mlp_net(w_bg_pSp)
				latent_t_c, latent_t_s1, latent_t_s2 = self.cs_mlp_net(w_t_pSp) 

				rec_x_bg = self.pSp_net.forward(latent_bg_c + latent_bg_s1, input_code=True, randomize_noise=True, recon_modle=True)
				rec_x_t = self.pSp_net.forward(latent_t_c + latent_t_s2, input_code=True, randomize_noise=True, recon_modle=True)	

				# Calculate loss
				loss_lat, loss_lat_dict = self.calc_latent_loss(latent_bg_c, latent_bg_s1, latent_bg_s2, latent_t_c, latent_t_s1, latent_t_s2, w_bg_pSp, w_t_pSp)
				...
				loss_adv, loss_adv_msg = calc_adv_loss(
					disc_net=self.disc_net,
				...
				loss = loss_lat + loss_img_bg + loss_img_t + loss_adv
				...
				if self.global_step % self.opts.image_interval == 0 :
					self.parse_and_log_images(id_logs_bg, x_bg, rec_x_pSp_bg, rec_x_bg, title='images/train/faces_bg')
					self.parse_and_log_images(id_logs_t, x_t, rec_x_pSp_t, rec_x_t, title='images/train/faces_t')
				...
				if self.global_step % 25 == 0 and self.n_print < 100:
					...
					self.n_print += 1
				if self.global_step % self.opts.log_adv_interval == 0:
					#self.write_metrics_to_txt(loss_adv_msg, prefix='train', filename='adv_losses.txt')
					write_log_to_txt(loss_adv_msg, self.opts.exp_dir, filename="train_adv_losses.txt")
				# Validation related
				val_loss_dict = None
				...
				if self.global_step % self.opts.train_disc_interval == 0:
					
					train_common_disc(self.disc_net, self.pSp_net, self.cs_mlp_net,
						self.train_bg_dataloader, self.train_t_dataloader,
						self.test_bg_dataloader, self.test_t_dataloader,
						self.disc_criterion, self.optimizer_disc, self.opts, self.device, self.global_step)
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_t_dataloader)):
			...
			with torch.no_grad():
				...
				# Calculate loss
				_, loss_lat_dict = self.calc_latent_loss(latent_bg_c, latent_bg_s1, latent_bg_s2, latent_t_c, latent_t_s1, latent_t_s2, w_bg_pSp, w_t_pSp)
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s1, latent_bg_s2, latent_t_c, latent_t_s1, latent_t_s2, w_bg_pSp, w_t_pSp):

		loss_dict = {}
		loss = 0.0

		if self.opts.s_zeros_lambda > 0:
	
			loss_bg_s2 = F.mse_loss(latent_bg_s2, torch.zeros_like(latent_bg_s2).to(self.device))
			loss_t_s1 = F.mse_loss(latent_t_s1, torch.zeros_like(latent_t_s1).to(self.device))
			loss_dict['loss_bg_s2'] = float(loss_bg_s2)
			loss_dict['loss_t_s1'] = float(loss_t_s1)
			loss += (loss_bg_s2 + loss_t_s1) * self.opts.s_zeros_lambda
			
		if self.opts.lat_recon_lambda > 0:
			loss_lat_recon_bg = F.mse_loss(latent_bg_c + latent_bg_s1, w_bg_pSp)
			loss_lat_recon_t = F.mse_loss(latent_t_c + latent_t_s2, w_t_pSp)
			loss_dict['loss_lat_recon_bg'] = float(loss_lat_recon_bg)
			loss_dict['loss_lat_recon_t'] = float(loss_lat_recon_t)
			loss += (loss_lat_recon_bg + loss_lat_recon_t) * self.opts.lat_recon_lambda	
