...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.optimizer, self.opt_D, self.opt_R1, self.opt_R2 = self.configure_optimizers()
	...
	def init_networks(self, cs_ckpt_path=None):
		...
		# -----------------------------
		self.domain_R1 = ReconR(
			latent_dim=512,
			hidden_dim=512,
			n_layers=self.opts.num_R_layers,
		).to(self.device)
		self.domain_R2 = ReconR(
			latent_dim=512,
			hidden_dim=512,
			n_layers=self.opts.num_R_layers,
		).to(self.device)
	...
	def get_R_and_opt(self, salient):
		if salient == "s1":
			return self.domain_R1, self.opt_R1
		elif salient == "s2":
			return self.domain_R2, self.opt_R2
		else:
			raise ValueError(f"Unknown salient: {salient}")

	def train_R(self, latent_c, latent_s, salient="s1"):
		R, opt_R = self.get_R_and_opt(salient)
		R.train()
		opt_R.zero_grad()

		s_hat = R(latent_c.detach())
		loss = F.mse_loss(s_hat, latent_s.detach())

		loss.backward()
		opt_R.step()

		return {f"R_{salient}_loss": loss.item()}
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, (batch_bg, batch_t) in enumerate(
				...
				if self.opts.R_lambda > 0 and self.global_step >= self.opts.R_start_step:
					log_R1 = {}
					log_R1.update(self.train_R(latent_X_c, latent_X_s1, salient="s1"))
					if not self.opts.no_train_Rnull:
						log_R1.update(self.train_R(latent_Y_c, latent_Y_s1, salient="s1"))
					monitorR["R1"] = log_R1

					log_R2 = {}
					if not self.opts.no_train_Rnull:
						log_R2.update(self.train_R(latent_X_c, latent_X_s2, salient="s2"))
					log_R2.update(self.train_R(latent_Y_c, latent_Y_s2, salient="s2"))
					monitorR["R2"] = log_R2
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(
			...
			with torch.no_grad():
				...
				# =====================================================
				logits_x = self.domain_D(latent_X_c)
				logits_y = self.domain_D(latent_Y_c)
	...
	def configure_optimizers(self):
		...
		opt_R1 = torch.optim.Adam(self.domain_R1.parameters(), lr=getattr(self.opts, "r_lr", 1e-4))
		opt_R2 = torch.optim.Adam(self.domain_R2.parameters(), lr=getattr(self.opts, "r_lr", 1e-4))

		return optimizer, opt_D, opt_R1, opt_R2
	...
	def R_cs_loss(self, latent_c, latent_s, salient = "s1"):
		"""
	...
	def calc_latent_loss(self, latent_X_c, latent_X_s1, latent_X_s2, latent_Y_c, latent_Y_s1, latent_Y_s2, w_X_pSp, w_Y_pSp):
		...
		if self.opts.sbg_lambda > 0:
			...
			loss_dict['silent_y'] = float(loss_silent_y)
		...
		if self.opts.D_lambda > 0 and self.global_step >= self.opts.D_start_step:
			...
			loss_D_raw = self.D_cxcy_loss(latent_X_c, latent_Y_c)
		...
		if self.opts.R_lambda > 0 and self.global_step >= self.opts.R_start_step:
			...
			# ---------- R1 (s1) ----------
			loss_R1 = self.R_cs_loss(latent_X_c, latent_X_s1, salient="s1")
			if not self.opts.no_train_Rnull:
				loss_R1 = loss_R1 + self.R_cs_loss(latent_Y_c, latent_Y_s1, salient="s1")

			# ---------- R2 (s2) ----------
			loss_R2 = self.R_cs_loss(latent_Y_c, latent_Y_s2, salient="s2")
			if not self.opts.no_train_Rnull:
				loss_R2 = loss_R2 + self.R_cs_loss(latent_X_c, latent_X_s2, salient="s2")
    
			loss_R_raw = loss_R1 + loss_R2
			loss_R = torch.clamp(loss_R_raw, min=-10.0, max=10.0)

			loss_dict['R_adv_s1'] = float(loss_R1.detach())
			loss_dict['R_adv_s2'] = float(loss_R2.detach())
