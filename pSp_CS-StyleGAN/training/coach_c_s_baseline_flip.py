...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.train_t_dataloader = DataLoader(self.train_bg_dataset,
										   batch_size=self.opts.batch_size,
										   shuffle=True,
										   num_workers=int(self.opts.workers),
										   drop_last=True)
		
		self.train_bg_dataloader = DataLoader(self.train_t_dataset,
									batch_size=self.opts.batch_size,
									shuffle=True,
									num_workers=int(self.opts.workers),
									drop_last=True)
		
		self.test_t_dataloader = DataLoader(self.test_bg_dataset,
										  batch_size=self.opts.test_batch_size,
										  shuffle=False,
										  num_workers=int(self.opts.test_workers),
										  drop_last=True)
		
		self.test_bg_dataloader = DataLoader(self.test_t_dataset,
										  batch_size=self.opts.test_batch_size,
										  shuffle=False,
										  num_workers=int(self.opts.test_workers),
										  drop_last=True)		
		self.special_batch_y, self.special_batch_x = get_special_images(self.device, size=self.opts.transform_size, datasets=self.opts.dataset_type)
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp):
		...
		if self.opts.lat_recon_lambda > 0:
			loss_lat_recon_bg = F.mse_loss(latent_bg_c, w_bg_pSp)
