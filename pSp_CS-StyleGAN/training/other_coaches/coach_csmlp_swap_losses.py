...
class Coach_csmlp:
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				swap_w_bg = self.pSp_net.forward(swap_x_bg, encode_only=True)
				swap_w_t = self.pSp_net.forward(swap_x_t, encode_only=True) 

				# Calculate loss
				loss_lat, loss_lat_dict = self.calc_latent_loss(latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp, swap_w_bg, swap_w_t)
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_t_dataloader)):
			...
			with torch.no_grad():
				...
				# Calculate loss
				_, loss_lat_dict = self.calc_latent_loss(latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp, swap_w_bg, swap_w_t)
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp, swap_w_bg, swap_w_t):
		"""
		...
		if self.opts.w_dist_swap_lambda > 0:
			loss_swap_dist_bg = F.mse_loss(latent_bg_c + latent_t_s, swap_w_bg)
			loss_swap_dist_t = F.mse_loss(latent_t_c, swap_w_t)
