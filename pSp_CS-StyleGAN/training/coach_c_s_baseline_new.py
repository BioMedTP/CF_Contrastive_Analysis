...
class Coach_csmlp:
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				if self.global_step % self.opts.image_interval == 0 :
					with torch.no_grad():
						swap_x_bg = self.pSp_net.forward(latent_bg_c + latent_t_s, input_code=True, randomize_noise=True, recon_modle=True)
						swap_x_t = self.pSp_net.forward(latent_t_c, input_code=True, randomize_noise=True, recon_modle=True)	
					self.parse_and_log_images(id_logs_bg, x_bg, rec_x_pSp_bg, rec_x_bg, swap_x_bg, title='images/train/faces_bg')
					self.parse_and_log_images(id_logs_t, x_t, rec_x_pSp_t, rec_x_t, swap_x_t, title='images/train/faces_t')
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp):
		...
		if self.opts.s_zeros_lambda > 0:
			...
			latent_bg_s = F.mse_loss(latent_bg_s, torch.zeros_like(latent_bg_s).to(self.device))
			
			loss_dict['loss_sx'] = float(latent_bg_s)
			loss += latent_bg_s * self.opts.s_zeros_lambda
	...
	def parse_and_log_images(self, id_logs, x, y_pSp, y_ours, y_swap, title, subscript=None, display_count=2):
		im_data = []
		for i in range(display_count):
			cur_im_data = {
				'input_face': common.log_input_image(x[i], self.opts),
				'pSp_recon_face': common.tensor2im(y_pSp[i]),
				'Our_recon_face': common.tensor2im(y_ours[i]),
				'Our_swap_face': common.tensor2im(y_swap[i])
			}
