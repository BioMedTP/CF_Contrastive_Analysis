...
from models.regressior import MI_Discriminator, ReconR
import math
...
def get_paired_random_batch(loader_X, loader_Y):
	...
	print(f"--------------------------------------------------Selected random batch index: {idx}")
...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.optimizer, self.opt_D, self.opt_R = self.configure_optimizers()
	...
	def init_networks(self, cs_ckpt_path=None):
		...
		# -----------------------------
		# MI Discriminator / Regressor R
		# -----------------------------
		if self.opts.R_mode == 'DiscMI':
			self.domain_R = MI_Discriminator(
				style_dim=self.opts.style_dim,
				latent_dim=512,
				num_layers=self.opts.num_R_layers,
			).to(self.device)
		elif self.opts.R_mode == 'RegrR':
			self.domain_R = ReconR(
				latent_dim=512,
				hidden_dim=512,
				n_layers=self.opts.num_R_layers,
			).to(self.device)
		else:
			raise NotImplementedError(f"Unknown R_mode: {self.opts.R_mode}")
	...
	def train_R(self, latent_c, latent_s):
		"""
		Train dependency estimator R on a single domain batch.

		Supports:
			- DiscMI: MI discriminator (joint vs marginal)
			- RegrR: regression C -> S
		"""
		self.domain_R.train()
		...
		# ==================================================
		# Case 1: Discriminative MI estimator
		# ==================================================
		if self.opts.R_mode == 'DiscMI':

			B = latent_c.size(0)
			perm = torch.randperm(B, device=latent_c.device)
			latent_s_perm = latent_s[perm]

			logits_pos = self.domain_R(latent_c.detach(), latent_s.detach())
			logits_neg = self.domain_R(latent_c.detach(), latent_s_perm.detach())
			...
			with torch.no_grad():
				acc_pos = (torch.sigmoid(logits_pos) > 0.5).float().mean()
				acc_neg = (torch.sigmoid(logits_neg) < 0.5).float().mean()
				acc_R = 0.5 * (acc_pos + acc_neg)

			self.last_R_acc = acc_R.item()
		...
		# ==================================================
		# Case 2: Regression-based dependency estimator
		# ==================================================
		elif self.opts.R_mode == 'RegrR':

			s_hat = self.domain_R(latent_c.detach())
			loss_R = F.mse_loss(s_hat, latent_s.detach())
			...
			raise ValueError(f"Unknown R_mode: {self.opts.R_mode}")
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, (batch_bg, batch_t) in enumerate(
				...
				# =====================================================
				# 3. Train discriminator D and R
				# =====================================================
				if self.opts.D_lambda > 0 and self.global_step >= self.opts.D_start_step:
					self.train_D(latent_X_c, latent_Y_c)
     
				monitorR = {}
				if self.opts.R_lambda > 0 and self.global_step >= self.opts.R_start_step:

					if not self.opts.no_train_Rx:
						log_Rx = self.train_R(latent_X_c, latent_X_s)
						monitorR["Rx"] = log_Rx

					log_Ry = self.train_R(latent_Y_c, latent_Y_s)
					monitorR["Ry"] = log_Ry
				...
				if self.global_step % 50 == 0 and self.global_step >= self.opts.D_start_step:
				...
				with torch.no_grad():
					...
					train_loss_dict["MonitorD"].update({
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(
			...
			with torch.no_grad():
				...
				_, loss_img_X_dict, _ = self.calc_image_loss(X, rec_X)
				_, loss_img_Y_dict, _ = self.calc_image_loss(Y, rec_Y)

				agg_loss_lat.append(loss_lat_dict)
				agg_loss_img_X.append(loss_img_X_dict)
				agg_loss_img_Y.append(loss_img_Y_dict)

				# =====================================================
				# DOMAIN-INVARIANCE MONITORING (diagnostic only)
				# =====================================================
		...
		loss_img_X = train_utils.aggregate_loss_dict(agg_loss_img_X)
		loss_img_Y = train_utils.aggregate_loss_dict(agg_loss_img_Y)
		...
		# =====================================================
		# unified validation dict (MATCHES train schema)
		# =====================================================
		val_loss_dict = {
			"LOSS_lat": loss_lat,
			...
			"Monitor": {
				"D_acc": float(np.mean(agg_D_acc)),
				"c_mean_dist": float(np.mean(agg_mean_dist)),
				"c_var_dist": float(np.mean(agg_var_dist)),
				"c_domain_gap": float(np.mean(agg_domain_gap)),
			}
		}
	...
	def configure_optimizers(self):
		...
		opt_D = torch.optim.Adam(self.domain_D.parameters(), lr=getattr(self.opts, "d_lr", 1e-4))
		opt_R = torch.optim.Adam(self.domain_R.parameters(), lr=getattr(self.opts, "r_lr", 1e-4))

		return optimizer, opt_D, opt_R
	...
	def D_cxcy_loss(self, latent_cx, latent_cy):
		"""
		...
		if self.opts.D_mode == 'confusion':
			...
			raise ValueError(f"Unknown D_mode: {self.opts.D_mode}")

		return loss_x + loss_y
		
	def R_cs_loss(self, latent_c, latent_s):
		"""
		CS-side loss to reduce dependency between C and S,
		according to the chosen R_mode.
		"""

		# ----------------------------------
		# Case 1: DiscMI → confusion loss
		# ----------------------------------
		if self.opts.R_mode == 'DiscMI':
			logits = self.domain_R(latent_c, latent_s)
			target = torch.full_like(logits, 0.5)  # confusion
			loss = F.binary_cross_entropy_with_logits(logits, target)
			return loss

		# ----------------------------------
		# Case 2: RegrR → block predictability
		# ----------------------------------
		elif self.opts.R_mode == 'RegrR':
			s_hat = self.domain_R(latent_c)
			# maximize reconstruction error to discourage predictability
			loss = -F.mse_loss(s_hat, latent_s)
			return loss
	...
	def calc_latent_loss(self, latent_cx, latent_sx, latent_cy, latent_sy, w_x_pSp, w_y_pSp):
		...
		if self.opts.D_lambda > 0 and self.global_step >= self.opts.D_start_step:
			...
			loss_D_raw = self.D_cxcy_loss(latent_cx, latent_cy)
			# loss_dict['D_adv_c_raw'] = float(loss_D_raw.detach())

			loss_adv = torch.clamp(loss_D_raw, max=10.0)
			loss_dict['D_adv_c'] = float(loss_adv.detach())

			loss += self.opts.D_lambda * loss_adv

		if self.opts.R_lambda > 0 and self.global_step >= self.opts.R_start_step:
			if not self.opts.no_train_Rx:
				loss_Rx = self.R_cs_loss(latent_cx, latent_sx)
			else:
				loss_Rx = 0.0
			loss_Ry = self.R_cs_loss(latent_cy, latent_sy)

			loss_R_raw = loss_Rx + loss_Ry
			loss_R = torch.clamp(loss_R_raw, min=-10.0, max=10.0)

			loss_dict['R_adv_x'] = float(loss_Rx.detach())
			loss_dict['R_adv_y'] = float(loss_Ry.detach())
			loss += self.opts.R_lambda * loss_R
