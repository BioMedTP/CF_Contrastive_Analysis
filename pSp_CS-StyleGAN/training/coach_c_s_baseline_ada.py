...
class Coach_csmlp:
	...
	def inference_special(self, X, Y, name):
		...
		with torch.no_grad():
			...
			rec_X = self.pSp_net.forward(latent_X_c, input_code=True, recon_model=True)
			rec_Y = self.pSp_net.forward(latent_Y_c + latent_Y_s, input_code=True, recon_model=True)        
			swap_cs_X = self.pSp_net.forward(latent_X_c + latent_Y_s, input_code=True, recon_model=True)
			swap_cs_Y = self.pSp_net.forward(latent_Y_c, input_code=True, recon_model=True)    
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_x, batch_y) in enumerate(zip(self.train_X_dataloader, self.train_Y_dataloader)):
				...
				# Calculate loss
				loss_lat, loss_lat_dict = self.calc_latent_loss(latent_X_c, latent_X_s, latent_Y_c, latent_Y_s, w_X_pSp, w_Y_pSp)
	...
	def validate(self):
		...
		for batch_idx, (batch_x, batch_y) in enumerate(zip(self.test_X_dataloader, self.test_Y_dataloader)):
			...
			with torch.no_grad():
				...
				# Calculate loss
				_, loss_lat_dict = self.calc_latent_loss(latent_X_c, latent_X_s, latent_Y_c, latent_Y_s, w_X_pSp, w_Y_pSp)
	...
	def merge_loss_dict(self, loss_lat_dict, loss_img_dict_bg, loss_img_dict_t):
		...
		loss_dict['loss_img_x'] = loss_img_dict_bg
		loss_dict['loss_img_y'] = loss_img_dict_t
