...
from utils.data_utils import get_special_paired_images
...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.test_t_dataloader = DataLoader(self.test_t_dataset,
										  ...
										  num_workers=int(self.opts.test_workers),
										  drop_last=True)		
		self.special_batch_x1, self.special_batch_y1, self.special_batch_x2, self.special_batch_y2 = get_special_paired_images(self.device, size=self.opts.transform_size, datasets=self.opts.dataset_type)
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				if self.global_step % self.opts.image_interval == 0 :
					...
					self.inference_special(self.special_batch_x1, self.special_batch_y1, name="val_glasses")
					self.inference_special(self.special_batch_x2, self.special_batch_y2, name="val_smile")
