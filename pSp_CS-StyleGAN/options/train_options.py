...
class TrainOptions:
	...
	def initialize(self):
		...
		self.parser.add_argument('--seed', default=42, type=int, help='Set the seed for reproducibility')
		self.parser.add_argument('--data_transform', default='grayscale256', type=str, help='input data transform type')
		self.parser.add_argument('--resize2pSp', default=True, type=bool, help='input size of pSp encoder')
		...
		self.parser.add_argument('--stylegan_size', default=1024, type=int, help='Output size of generator')
		...
		self.parser.add_argument('--decoder_multiplier', default=1, type=int, help='Multiplier for decoder')
		...
		self.parser.add_argument(
			...
			type=str,
			default=["t1n", "t1c"],
			help="Specify the modalities for X and Y, e.g. --XY_mods t1n t1c"
		)
		...
		self.parser.add_argument(
			...
			help="Whether to use paired or unpaired multi-modality splits"
		)
		self.parser.add_argument(
			"--train_shuffule",
			action="store_true",
			help="Whether to shuffle the training data during training"
		)

		self.parser.add_argument("--random_flip", action="store_true", help="Whether to apply random flipping augmentation to the images during training")

		self.parser.add_argument('--batch_size', default=4, type=int, help='Batch size for training')
		self.parser.add_argument('--transform_size', default=256, type=int)
		self.parser.add_argument('--workers', default=4, type=int, help='Number of train dataloader workers')
		self.parser.add_argument('--test_batch_size', default=4, type=int, help='Number of test/inference dataloader batchsize')
		self.parser.add_argument('--test_workers', default=4, type=int, help='Number of test/inference dataloader workers')
		self.parser.add_argument('--pSp_encoder_type', default='GradualStyleEncoder', type=str, help='pSp models to use, choices: GradualStyleEncoder, BackboneEncoderUsingLastLayerIntoW, BackboneEncoderUsingLastLayerIntoWPlus')
		self.parser.add_argument('--resume_ckpt_path', type=str, default=None, help='Path to checkpoint for resuming training cs network.')

		self.parser.add_argument('--train_pSp_encoder', action='store_true', help='Whether to train the decoder model')		
		self.parser.add_argument('--train_stylegan_decoder', action='store_true', help='Whether to train the decoder model')
		...
		self.parser.add_argument('--mlp_norm_type', default='none', type=str, help='Which type of weight to use, choices: 2D or 3D')		
		...
		# -------------------------------
		# Discriminator (Domain D)
		# -------------------------------
		self.parser.add_argument('--num_D_layers', default=1, type=int,
								help='Number of layers in domain discriminator.')
		self.parser.add_argument('--D_lr', default=2e-4, type=float,
								help='Learning rate for domain discriminator.')
		self.parser.add_argument('--D_lambda', default=0.05, type=float,
								help='Adversarial loss weight for domain discriminator.')
		self.parser.add_argument('--D_start_step', default=10000, type=int,
								help='Warm-up step before training discriminator.')
		self.parser.add_argument('--D_mode', default='confusion', type=str,
								choices=['confusion', 'flip'],
								help='Adversarial objective for discriminator.')

		# -------------------------------
		# Regressor (R)
		# -------------------------------
		self.parser.add_argument('--num_R_layers', default=1, type=int,
								help='Number of layers in regressor.')
		self.parser.add_argument('--R_lr', default=2e-4, type=float,
								help='Learning rate for regressor.')
		self.parser.add_argument('--R_lambda', default=0.05, type=float,
								help='Adversarial loss weight for regressor.')
		self.parser.add_argument('--R_start_step', default=10000, type=int,
								help='Warm-up step before training regressor.')
		self.parser.add_argument('--R_mode', default='confusion', type=str,
								help='Adversarial objective for regressor.')
		self.parser.add_argument('--no_train_Rx', action='store_true',
								help='Disable training R on X modality.')
		self.parser.add_argument('--no_train_Rnull', action='store_true',
								help='Disable training R on X modality.')
  
		self.parser.add_argument('--special_idx', default=-1, type=int)
		self.parser.add_argument('--resize_pSp_output', action='store_true')
		...
		self.parser.add_argument('--w_dist_lambda', default=1.0, type=float) 
		...
		self.parser.add_argument('--l1_lambda', default=0.0, type=float, help='L1 loss multiplier factor')
		self.parser.add_argument('--ssim_lambda', default=0.0, type=float, help='ssim loss multiplier factor')
		self.parser.add_argument('--msssim_lambda', default=0.0, type=float, help='msssim loss multiplier factor')
		# self.parser.add_argument('--w_norm_lambda', default=0.0, type=float, help='W-norm loss multiplier factor')
		self.parser.add_argument('--sbg_lambda', default=1.0, type=float, help='l1 loss for silent factor of target')
		self.parser.add_argument('--lat_recon_lambda', default=1.0, type=float)

		# self.parser.add_argument('--ssim_lambda', default=0.0, type=float, help='similarity loss for latent w')
		...
		self.parser.add_argument('--csmlp_checkpoint_path', default=None, type=str, help='Path to csmlp model checkpoint')
		...
		self.parser.add_argument('--e4e_checkpoint_path', default="None", type=str, help='Path to e4e model checkpoint')
		# self.parser.add_argument('--disc_checkpoint_path', default=None, type=str, help='Path to discriminator checkpoint')
		...
		self.parser.add_argument('--log_interval', default=1000, type=int, help='Interval for logging metrics to text file')
