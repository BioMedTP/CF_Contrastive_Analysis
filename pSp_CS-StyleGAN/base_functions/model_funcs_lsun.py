...
def process_recon_swap(cs_mlp_net, pSp_net, input_images_bg, input_images_t, opts):
    ...
    with torch.no_grad():
        ...
        recon_bg, _ = pSp_net.decoder([latent_bg_c], randomize_noise=False, input_is_latent=True)
        recon_t, _ = pSp_net.decoder([latent_t_c + latent_t_s], randomize_noise=False, input_is_latent=True)	

        swap_bg, _ = pSp_net.decoder([latent_bg_c + latent_t_s], randomize_noise=False, input_is_latent=True)
        swap_t, _ = pSp_net.decoder([latent_t_c ], randomize_noise=False, input_is_latent=True)	
