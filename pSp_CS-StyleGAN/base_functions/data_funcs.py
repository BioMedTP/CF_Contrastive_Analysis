...
#     """
#     if opts.dataset_type not in data_configs.Medical_DATASETS.keys():
#         raise ValueError(f"{opts.dataset_type} is not a valid dataset_type")

#     print(f"Loading dataset for {opts.dataset_type}")
#     dataset_args = data_configs.Medical_DATASETS[opts.dataset_type]
...
def configure_datasets(opts):
    ...
    # ---------------------------
    if opts.dataset_type not in data_configs.Medical_DATASETS:
        raise Exception(f'{opts.dataset_type} is not a valid dataset_type')

    print(f'Loading dataset for {opts.dataset_type}')

    # Copy base config
    dataset_args = dict(data_configs.Medical_DATASETS[opts.dataset_type])
    transforms_dict = dataset_args['transforms'](opts).get_transforms()
    # ---------------------------
    # 2. Handle special case: bratsMultiMod
    # ---------------------------
    if opts.dataset_type == "bratsMultiMod":

        X_mod, Y_mod = opts.XY_mods      # e.g. ["t1n","t1c"]
        mode = opts.pairing              # "paired" or "unpaired"
    ...
    def load(path):
        return ImagesDataset_mednpy(
            npy_path=path,
            transform=transforms_dict[opts.data_transform],
            opts=opts
        )
...
def perform_inference_to_hdf5(bg_dataloader, t_dataloader, cs_mlp_net, pSp_net, save_path="latents.h5", device="cuda"):
    ...
    with h5py.File(save_path, "w") as f:
        ...
        with torch.no_grad():
            ...
            for x_bg in tqdm(bg_dataloader, desc="Processing BG Latents"):
            ...
            for x_t in tqdm(t_dataloader, desc="Processing T Latents"):
