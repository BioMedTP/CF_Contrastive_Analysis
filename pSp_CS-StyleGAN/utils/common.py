...
def vis_faces(log_hooks):
	...
	fig = plt.figure(figsize=(16, 4 * display_count))  # Wider figsize for 4 columns
	gs = fig.add_gridspec(display_count, 4)            # ✅ Fix column count here
...
# def vis_faces(log_hooks):
# 	display_count = len(log_hooks)
# 	image_keys = list(log_hooks[0].keys())
# 	n_cols = sum(isinstance(log_hooks[0][k], Image.Image) for k in image_keys)

# 	fig = plt.figure(figsize=(4 * n_cols, 4 * display_count))
# 	gs = fig.add_gridspec(display_count, n_cols)

# 	for i in range(display_count):
# 		hooks_dict = log_hooks[i]
# 		img_col = 0
# 		for key, val in hooks_dict.items():
# 			if isinstance(val, Image.Image):  # Only plot images
# 				ax = fig.add_subplot(gs[i, img_col])
# 				ax.imshow(val)
...
def vis_faces_no_id(hooks_dict, fig, gs, i):
	...
	plt.imshow(hooks_dict['Our_swap_face'])
	plt.title('Swapped')

def vis_faces_with_id(hooks_dict, fig, gs, i):
	fig.add_subplot(gs[i, 0])
	plt.imshow(hooks_dict['input_face'])
	plt.title('Input\nSim={:.2f}'.format(float(hooks_dict.get('diff_input', 0.0))))

	fig.add_subplot(gs[i, 1])
	plt.imshow(hooks_dict['pSp_recon_face'])
	plt.title('pSp\nSim={:.2f}'.format(float(hooks_dict.get('diff_target', 0.0))))

	fig.add_subplot(gs[i, 2])
	plt.imshow(hooks_dict['Our_recon_face'])
	plt.title('Ours\nSim={:.2f}'.format(float(hooks_dict.get('diff_target', 0.0))))
