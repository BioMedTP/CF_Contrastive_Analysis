    ...
    for epoch in range(args.max_epochs):
        ...
        for step, (latent_bg_c, latent_bg_s, latent_t_c, latent_t_s) in enumerate(train_dataloader):
            ...
            with torch.amp.autocast(device_type=device_type):  # ✅ Enable Mixed Precision
                ...
                loss_unscaled = args.lambda_bg * loss_bg + args.lambda_t * loss_t  # ✅ Keep unscaled loss for tracking
    ...
    with torch.no_grad():
        for latent_bg_c, latent_bg_s, latent_t_c, latent_t_s in val_dataloader:
            ...
            with torch.amp.autocast(device_type=device_type):  # ✅ Enable AMP for validation too
                ...
                loss = args.lambda_bg * loss_bg + args.lambda_t * loss_t
...
def main():
    ...
    parser.add_argument('--lambda_bg', type=float, default=config.get('lambda_bg', 1.0))
    parser.add_argument('--lambda_t', type=float, default=config.get('lambda_t', 1.0))
    parser.add_argument('--prev_train_path', type=str, default=config.get('prev_train_path', None))
    parser.add_argument('--normalize', action='store_true', default=config.get('normalize', False))
