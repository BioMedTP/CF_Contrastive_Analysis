...
class MI_Discriminator(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        input_dim  = self.args.latent_dim * 2   # 1024 if 512-d latents concatenated
        hidden_dim = 512
        num_layers = self.args.n_layers         # e.g., 1 means no hidden layers

        layers = []
        prev_dim = input_dim

        # add (num_layers - 1) hidden layers of size hidden_dim
        for _ in range(max(0, num_layers - 1)):
            layers.append(nn.Linear(prev_dim, hidden_dim))
            layers.append(nn.ReLU())
            prev_dim = hidden_dim

        # final layer FROM prev_dim -> 1  (handles both num_layers==1 and >=2)
        layers.append(nn.Linear(prev_dim, 1))

        self.mlp = nn.Sequential(*layers)

    def forward(self, c, s):
        # optional: stabilize with light normalization
        # c = F.layer_norm(c, c.shape[-1:])
        # s = F.layer_norm(s, s.shape[-1:])

        x = torch.cat([c, s], dim=-1)  # [B, 18, 1024] if latent_dim=512
        x = self.mlp(x)                # [B, 18, 1]
        x = x.mean(dim=1)              # [B, 1]
