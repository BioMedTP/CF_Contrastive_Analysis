...
After training the separator `H_cs`, we can train the refinement module using latent factors produced by the frozen encoders `E` and `H_cs`, and the corresponding reconstructions generated by `G` from the common/salient latents. We refine swapped outputs in the generator intermediate feature space (F-space) using feature shifts induced by common/salient manipulation. In our implementation, the F-space feature corresponds to the output feature map of the StyleGAN2 generator layer with 0-based index 8, i.e., the 9-th convolutional layer.
